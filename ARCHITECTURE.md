...
- Thumbnail images are mocked as backend-provided image payloads through an API boundary and rendered directly in cards.
...
4. `src/api/mockCharts.ts` composes chart metadata and asks `src/api/mockThumbnailBackend.ts` for mock backend thumbnail images.
5. `src/api/mockCharts.ts` returns normalized mock `ChartThumbnail[]` records with backend-image URLs and feature values.
...
- Current thumbnail API is mocked in-browser and does not validate real backend transfer cost, auth, or caching behavior yet.
